...
- [ ] **Sprint 2: Recurring health & refund-aware insights**
  - Track recurring state (active/paused/failed) and next_charge_date from ActBlue; surface churn/recurring health KPIs.
  - Wire chargebacks/failed payments (if available) and net-LTV; extend channel/attribution views with refund overlays (fundraising chart already net/refund-aware).
...
- Add recurring_state/next_charge_date fields and ingestion; churn/recurring KPIs in dashboard.
- Model chargebacks/failed payments if available; extend channel/attribution views with refund overlays.
- (If desired) add deterministic-only toggle to channel charts and Donation list/search.
