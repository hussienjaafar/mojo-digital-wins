...
export function DashboardHome() {
  ...
  const fetchDashboardStats = async () => {
    try {
      // Query canonical sources directly instead of legacy daily_aggregated_metrics
      const orgsQuery = (supabase as any).from('client_organizations').select('id, is_active');

      // ActBlue canonical source - all transactions
      const actblueQuery = (supabase as any)
        .from('actblue_transactions_secure')
        .select('amount');

      // Meta ads canonical source
      const metaQuery = (supabase as any)
        .from('meta_ad_metrics')
        .select('spend');

      // SMS canonical source
      const smsQuery = (supabase as any)
        .from('sms_campaigns')
        .select('cost')
        .neq('status', 'draft');

      const [orgsResult, actblueResult, metaResult, smsResult] = await Promise.all([
        orgsQuery,
        actblueQuery,
        metaQuery,
        smsQuery,
      ...
      // Calculate totals from canonical sources
      const totalRevenue = actblueData.reduce((sum: number, t: any) => sum + (Number(t.amount) || 0), 0);
      const totalAdSpend = metaData.reduce((sum: number, m: any) => sum + (Number(m.spend) || 0), 0);
      const totalSmsSpend = smsData.reduce((sum: number, s: any) => sum + (Number(s.cost) || 0), 0);
      const totalSpend = totalAdSpend + totalSmsSpend;
      const totalDonations = actblueData.length;
  ...
  const fetchChartData = async () => {
    try {
      // Query canonical sources with date range filtering
      const [actblueResult, metaResult, smsResult] = await Promise.all([
        // ActBlue transactions - includes transaction_date for daily grouping
        (supabase as any)
          .from('actblue_transactions_secure')
        ...
        // Meta ad metrics - already grouped by date
        (supabase as any)
          .from('meta_ad_metrics')
          .select('date, spend')
          .gte('date', startDate)
      ...
      const actblueByDate: Record<string, { revenue: number; donations: number }> = {};
      actblueData.forEach((t: any) => {
        const dateKey = t.transaction_date?.split('T')[0];
        if (!dateKey) return;
        if (!actblueByDate[dateKey]) {
          actblueByDate[dateKey] = { revenue: 0, donations: 0 };
        }
        actblueByDate[dateKey].revenue += Number(t.amount) || 0;
        actblueByDate[dateKey].donations += 1;
      });
      ...
      // Build complete date range and combine data
      ...
      const formattedData: ChartDataPoint[] = sortedDates.map((dateKey) => {
        const actblue = actblueByDate[dateKey] || { revenue: 0, donations: 0 };
        const metaSpend = metaByDate[dateKey] || 0;
        const smsSpend = smsByDate[dateKey] || 0;
        const totalSpend = metaSpend + smsSpend;
        const roi = totalSpend > 0 ? ((actblue.revenue - totalSpend) / totalSpend) * 100 : 0;

        return {
          date: new Date(dateKey).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }),
          revenue: actblue.revenue,
          spend: totalSpend,
          roi,
          donations: actblue.donations,
        };
      });
