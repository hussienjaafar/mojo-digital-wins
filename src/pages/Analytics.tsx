...
import { CalendarIcon, Download, TrendingUp, FileText, Scale, Hash, AlertTriangle, Newspaper, Users } from "lucide-react";
          ...
          <Popover>
            ...
            <PopoverContent className="w-auto p-0" align="end">
              <div className="p-3 space-y-2">
                <Button variant="outline" size="sm" onClick={() => setDateRange({ from: subDays(new Date(), 1), to: new Date() })}>
                  Last 24h
                </Button>
                ...
                {topicSentiments.map((topic, index) => (
                  <div key={topic.topic} className="border-b pb-4 last:border-0">
                    ...
                    <div className="flex gap-1 h-8 rounded overflow-hidden">
                      ...
                      >
                        {topic.positive > 0 && topic.positive}
                      </div>
                      <div
                      ...
                      >
                        {topic.neutral > 0 && topic.neutral}
                      </div>
                      <div
                        className="bg-red-500 flex items-center justify-center text-white text-xs font-medium"
                        style={{ width: `${(topic.negative / topic.total) * 100}%` }}
                      >
                        {topic.negative > 0 && topic.negative}
                      </div>
                    </div>
