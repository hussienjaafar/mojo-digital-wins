-- First drop both versions of the function (there appear to be duplicates)
DROP FUNCTION IF EXISTS public.get_creative_intelligence(uuid, date, date, integer, integer, numeric);

-- Recreate with correct table reference (meta_ad_metrics_daily instead of meta_ad_daily_insights)
...
BEGIN
  -- Check if we have daily metrics data (CORRECT table name)
  SELECT EXISTS(
  ...
  WITH creative_performance AS (
    ...
    GROUP BY c.id, c.ad_id, c.organization_id, c.issue_primary, c.stance, 
             c.target_attacked, c.headline, c.primary_text, c.thumbnail_url, 
