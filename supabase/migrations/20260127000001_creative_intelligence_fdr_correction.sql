-- Benjamini-Hochberg FDR Correction for Creative Intelligence
-- Adds p-value calculation and FDR correction to issue_performance analysis

-- Helper function: Calculate p-value from z-score using normal distribution approximation
-- Uses the Abramowitz and Stegun approximation for the cumulative normal distribution
CREATE OR REPLACE FUNCTION calculate_p_value_from_z(z_score FLOAT)
RETURNS FLOAT
LANGUAGE plpgsql
IMMUTABLE
AS $$
...
BEGIN
  -- Handle edge cases
  IF z_score IS NULL THEN
    RETURN NULL;
  END IF;

  abs_z := ABS(z_score);

  -- For very large z-scores, return near-zero p-value
  IF abs_z > 8 THEN
    RETURN 0.0000001;
  END IF;
  ...
  phi := 0.39894228 * EXP(-0.5 * abs_z * abs_z) * (
    0.319381530 * t
    - 0.356563782 * t * t
    + 1.781477937 * t * t * t
    - 1.821255978 * t * t * t * t
...
-- Main function: Benjamini-Hochberg FDR correction
-- Takes an array of p-values and returns adjusted p-values
CREATE OR REPLACE FUNCTION benjamini_hochberg_correction(p_values FLOAT[])
RETURNS FLOAT[]
LANGUAGE plpgsql
IMMUTABLE
AS $$
...
BEGIN
  ...
  -- Initialize arrays
  sorted_indices := ARRAY[]::INT[];
  sorted_pvalues := ARRAY[]::FLOAT[];
  adjusted := ARRAY[]::FLOAT[];

  -- Create index array and sort by p-values
  -- We need to track original positions for proper adjustment
  FOR i IN 1..m LOOP
    sorted_indices := sorted_indices || i;
    sorted_pvalues := sorted_pvalues || COALESCE(p_values[i], 1.0);
    adjusted := adjusted || 0.0::FLOAT;
  END LOOP;

  -- Bubble sort to get indices sorted by p-value (ascending)
  FOR i IN 1..m LOOP
    FOR j IN 1..(m - i) LOOP
      IF sorted_pvalues[j] > sorted_pvalues[j + 1] THEN
        -- Swap p-values
        min_so_far := sorted_pvalues[j];
        sorted_pvalues[j] := sorted_pvalues[j + 1];
        sorted_pvalues[j + 1] := min_so_far;
        -- Swap indices
        adjusted_p := sorted_indices[j];
        sorted_indices[j] := sorted_indices[j + 1];
        sorted_indices[j + 1] := adjusted_p::INT;
      END IF;
    END LOOP;
  END LOOP;

  -- Apply BH correction from largest to smallest
  -- adjusted_p[i] = min(1, min(adjusted_p[i+1], m * p[i] / i))
  min_so_far := 1.0;

  FOR i IN REVERSE m..1 LOOP
    -- BH formula: m * p / rank
    adjusted_p := (m::FLOAT * sorted_pvalues[i]) / i::FLOAT;
    adjusted_p := LEAST(1.0, adjusted_p);
    adjusted_p := LEAST(min_so_far, adjusted_p);
...
-- Drop type if exists and recreate to handle type changes
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'fdr_result') THEN
    DROP TYPE fdr_result CASCADE;
  END IF;
END $$;

-- Table type for FDR results (used internally)
CREATE TYPE fdr_result AS (
  issue_primary TEXT,
  p_value FLOAT,
...
-- Updated Creative Intelligence RPC with FDR correction
...
BEGIN
  ...
  -- Issue performance with raw p-value calculation
  issue_performance_raw AS (
    SELECT
      ...
      -- Calculate standard error: SE = stddev / sqrt(n)
      CASE
        WHEN COUNT(*) > 1 AND STDDEV(roas) IS NOT NULL AND STDDEV(roas) > 0
        THEN STDDEV(roas) / SQRT(COUNT(*))
        ELSE NULL
      END as standard_error,
      -- Calculate z-score: z = (mean_roas - 1.0) / SE (testing if ROAS differs from 1.0)
      CASE
        WHEN COUNT(*) > 1 AND STDDEV(roas) IS NOT NULL AND STDDEV(roas) > 0
  ...
  -- Final issue performance with FDR correction
...
-- Grant permissions
GRANT EXECUTE ON FUNCTION calculate_p_value_from_z TO authenticated;
GRANT EXECUTE ON FUNCTION calculate_p_value_from_z TO service_role;
GRANT EXECUTE ON FUNCTION benjamini_hochberg_correction TO authenticated;
GRANT EXECUTE ON FUNCTION benjamini_hochberg_correction TO service_role;
...
COMMENT ON FUNCTION calculate_p_value_from_z IS 'Calculates two-tailed p-value from z-score using Abramowitz and Stegun normal distribution approximation';
COMMENT ON FUNCTION benjamini_hochberg_correction IS 'Applies Benjamini-Hochberg FDR correction to an array of p-values, returning adjusted p-values';
COMMENT ON FUNCTION get_creative_intelligence IS 'Returns creative intelligence analytics with FDR-corrected statistical significance for issue performance';
