-- Drop and recreate get_sms_metrics function with improved matching logic
...
BEGIN
  ...
  -- Get all potential SMS donations using the EXTRACTED refcode from SMS campaigns
  -- This matches the refcode in the SMS message URL to ActBlue transactions
  sms_donations AS (
    SELECT 
      t.id as txn_id,
  ...
  matched_donations AS (
    SELECT 
      ...
      d.txn_date,
      d.refcode,
      -- Calculate match score using tiered logic
      CASE 
        ...
        WHEN d.refcode ~ '^20[0-9]{6}[A-Za-z]+'
             ...
             AND (
               -- Date portion matches send_date within ±1 day
               ABS(
                 EXTRACT(EPOCH FROM (
                   TO_DATE(LEFT(d.refcode, 8), 'YYYYMMDD') - c.send_date_local
                 )) / 86400
               ) <= 1
             )
             AND (
               -- Token portion matches extracted_refcode (case-insensitive, partial)
        ...
        -- Tier 4: Date-coded refcode matches send_date only (no token match)
        WHEN d.refcode ~ '^20[0-9]{6}[A-Za-z]+'
             AND (
               ABS(
                 EXTRACT(EPOCH FROM (
  ...
  -- Select best match for each donation (highest score, earliest campaign if tie)
  best_matches AS (
  ...
  campaigns_with_metrics AS (
    SELECT 
      ...
      c.actblue_refcode,
      c.destination_url,
      -- Use calculated donations or fall back to stored values
      ...
      CASE WHEN COALESCE(c.recipients, 0) > 0 
           THEN ROUND((COALESCE(c.clicks, 0)::numeric / c.recipients) * 100, 2) 
