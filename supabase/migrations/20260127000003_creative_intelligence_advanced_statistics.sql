-- Advanced Statistical Features for Creative Intelligence
-- Adds: Effect size, statistical power, correlation significance, early window validation

-- =============================================================================
-- HELPER FUNCTIONS
-- =============================================================================

-- Calculate correlation p-value using t-distribution approximation
-- For correlation coefficient r with sample size n
CREATE OR REPLACE FUNCTION calculate_correlation_pvalue(r FLOAT, n INT)
RETURNS FLOAT
LANGUAGE plpgsql
IMMUTABLE
AS $$
...
BEGIN
  -- Handle edge cases
  IF r IS NULL OR n IS NULL OR n < 3 THEN
    RETURN NULL;
  END IF;

  -- Perfect correlation has p=0
  IF ABS(r) >= 0.9999 THEN
    RETURN 0.0000001;
  END IF;
  ...
  t_stat := r * SQRT(df::FLOAT) / SQRT(1.0 - r * r);
  abs_t := ABS(t_stat);

  -- Approximation of two-tailed t-distribution p-value
  -- Using the relationship with the beta incomplete function
  -- For large df (>30), t approaches normal distribution
  IF df > 30 THEN
    -- Use normal approximation via our existing p-value function
    RETURN calculate_p_value_from_z(t_stat);
  ELSE
    -- Use approximation for smaller df
    -- Based on Abramowitz & Stegun 26.7.5
    x := df / (df + t_stat * t_stat);

    -- Simplified beta approximation for common df values
    -- This gives reasonable accuracy for df between 1 and 30
    IF abs_t < 1.0 THEN
      p_value := 1.0 - 0.5 * (1.0 - x) * (1.0 + 0.5 * (df - 1) / (df + 1) * (1.0 - x));
    ELSIF abs_t < 2.0 THEN
      p_value := 0.5 * POWER(x, df / 2.0);
    ELSIF abs_t < 3.0 THEN
      p_value := 0.25 * POWER(x, df / 2.0);
    ELSE
      p_value := 0.1 * POWER(x, df / 2.0);
    END IF;

    -- Two-tailed p-value
    p_value := 2.0 * LEAST(p_value, 1.0 - p_value);
  END IF;

  RETURN GREATEST(0.0000001, LEAST(1.0, p_value));
END;
$$;

-- Calculate Cohen's d effect size for testing mean vs reference value
-- d = (mean - reference) / stddev
CREATE OR REPLACE FUNCTION calculate_cohens_d(mean_val FLOAT, reference_val FLOAT, stddev_val FLOAT)
RETURNS FLOAT
LANGUAGE plpgsql
IMMUTABLE
AS $$
BEGIN
  ...
  RETURN (mean_val - reference_val) / stddev_val;
END;
$$;

-- Estimate statistical power (simplified)
-- Based on effect size and sample size
-- Returns approximate power for detecting the observed effect
CREATE OR REPLACE FUNCTION estimate_statistical_power(effect_size FLOAT, sample_size INT, alpha FLOAT DEFAULT 0.05)
RETURNS FLOAT
LANGUAGE plpgsql
IMMUTABLE
AS $$
DECLARE
  ...
  power FLOAT;
BEGIN
  IF effect_size IS NULL OR sample_size IS NULL OR sample_size < 2 THEN
    RETURN NULL;
  END IF;
  ...
  IF alpha = 0.01 THEN
    z_alpha := 2.576;
  ELSIF alpha = 0.10 THEN
    z_alpha := 1.645;
  END IF;

  -- Non-centrality parameter: effect_size * sqrt(n)
  ncp := ABS(effect_size) * SQRT(sample_size::FLOAT);

  -- Power approximation using normal distribution
  -- Power ≈ Φ(ncp - z_alpha) for one-tailed, adjusted for two-tailed
  -- Using simplified formula: power ≈ Φ(|d|*sqrt(n) - z_α/2)

  IF ncp > z_alpha + 3 THEN
    power := 0.99;  -- Very high power
  ELSIF ncp > z_alpha + 2 THEN
    power := 0.95;
  ELSIF ncp > z_alpha + 1 THEN
    power := 0.84;
  ELSIF ncp > z_alpha THEN
    power := 0.50 + 0.34 * (ncp - z_alpha);
  ELSIF ncp > z_alpha - 1 THEN
    power := 0.16 + 0.34 * (ncp - z_alpha + 1);
  ELSE
    power := 0.05 + 0.11 * GREATEST(0, ncp);
  END IF;

  RETURN ROUND(LEAST(0.99, GREATEST(0.05, power))::numeric, 2);
END;
$$;

-- Interpret statistical power
CREATE OR REPLACE FUNCTION interpret_power(power FLOAT)
RETURNS TEXT
LANGUAGE plpgsql
IMMUTABLE
AS $$
BEGIN
  IF power IS NULL THEN
    RETURN 'Insufficient data';
  ELSIF power >= 0.80 THEN
    RETURN 'Adequate power - results are reliable';
  ELSIF power >= 0.50 THEN
    RETURN 'Moderate power - may miss real effects';
  ELSE
    RETURN 'Low power - need more data for reliable conclusions';
  END IF;
END;
$$;

-- Grant permissions on helper functions
GRANT EXECUTE ON FUNCTION calculate_correlation_pvalue TO authenticated;
GRANT EXECUTE ON FUNCTION calculate_correlation_pvalue TO service_role;
GRANT EXECUTE ON FUNCTION calculate_cohens_d TO authenticated;
GRANT EXECUTE ON FUNCTION calculate_cohens_d TO service_role;
GRANT EXECUTE ON FUNCTION estimate_statistical_power TO authenticated;
GRANT EXECUTE ON FUNCTION estimate_statistical_power TO service_role;
GRANT EXECUTE ON FUNCTION interpret_power TO authenticated;
GRANT EXECUTE ON FUNCTION interpret_power TO service_role;

-- =============================================================================
-- UPDATED RPC WITH ADVANCED STATISTICS
-- =============================================================================
...
BEGIN
  ...
  issue_performance_raw AS (
    SELECT
      ...
      CASE
        ...
        ELSE NULL
      END as z_score,
      -- Effect size (Cohen's d): how many standard deviations from ROAS=1.0
  ...
  -- Aggregate p-values for BH correction
  pvalue_array AS (
    SELECT
  ...
  -- Final issue performance with all statistics
  ...
  -- Early vs final with MINIMUM IMPRESSIONS validation
  early_vs_final AS (
    SELECT
      cp.creative_id,
      cp.roas as final_roas,
      -- Only calculate early CTR if we have enough impressions for stability
      (SELECT
        CASE
      ...
      -- Only calculate early CPM if we have enough impressions
      (SELECT
        CASE
          WHEN SUM(dm2.impressions) >= p_min_early_impressions
      ...
      -- Track early window impressions for transparency
    ...
    WHERE cp.days_with_data >= p_early_window_days + 3
  ),
  -- Leading indicators with correlation significance testing
  leading_indicators AS (
    SELECT
      ...
      -- Correlation p-values for significance testing
      ...
      -- Average early impressions for quality indicator
      ROUND(AVG(early_impressions)) as avg_early_impressions
    FROM early_vs_final
    WHERE early_ctr IS NOT NULL  -- Only include those with valid early metrics
  ),
  creative_trend AS (
  ...
  -- Summary statistics for FDR-corrected results
  fdr_summary AS (
    SELECT
...
-- Update comments
COMMENT ON FUNCTION get_creative_intelligence IS 'Returns creative intelligence analytics with FDR correction, effect sizes, statistical power, and correlation significance testing.';
COMMENT ON FUNCTION calculate_correlation_pvalue IS 'Calculates two-tailed p-value for Pearson correlation coefficient using t-distribution';
COMMENT ON FUNCTION calculate_cohens_d IS 'Calculates Cohen''s d effect size: (mean - reference) / stddev';
COMMENT ON FUNCTION estimate_statistical_power IS 'Estimates statistical power based on effect size and sample size';
COMMENT ON FUNCTION interpret_power IS 'Provides human-readable interpretation of statistical power value';
