...
-- Also fix the audit logs INSERT policy as flagged in security scan
DROP POLICY IF EXISTS "System can insert audit logs" ON public.admin_audit_logs;

-- Only authenticated admins can create audit logs (through functions with SECURITY DEFINER)
