-- Add minimum sample size requirement for statistical validity
-- Issues with < 3 creatives cannot produce valid standard deviation or z-scores

-- Updated Creative Intelligence RPC with minimum sample size filter
...
BEGIN
  ...
  -- CRITICAL: Require minimum sample size for valid statistics
  issue_performance_raw AS (
    ...
    -- MINIMUM SAMPLE SIZE: Require at least p_min_creatives_per_issue creatives for valid statistics
    -- With n<3, standard deviation is unreliable and z-tests are meaningless
...
-- Update comment
COMMENT ON FUNCTION get_creative_intelligence IS 'Returns creative intelligence analytics with FDR-corrected statistical significance. Requires minimum 3 creatives per issue for valid statistics.';
