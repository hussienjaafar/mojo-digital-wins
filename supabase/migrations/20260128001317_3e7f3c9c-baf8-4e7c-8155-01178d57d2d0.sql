-- Fix get_creative_intelligence function by removing video column references that don't exist
...
BEGIN
  ...
  stance_performance AS (
    SELECT jsonb_array_elements_text(political_stances::jsonb) as stance,
           COUNT(*) as creative_count,
    ...
    FROM creative_performance
    WHERE political_stances IS NOT NULL AND political_stances != '[]'
    GROUP BY jsonb_array_elements_text(political_stances::jsonb)
    HAVING COUNT(*) >= 2
  ),
  -- Target attacked performance
  target_performance AS (
    SELECT jsonb_array_elements_text(targets_attacked::jsonb) as target,
           COUNT(*) as creative_count,
           ROUND(AVG(roas)::numeric, 4) as mean_roas,
           ROUND(SUM(total_revenue)::numeric, 2) as total_revenue
    FROM creative_performance
    WHERE targets_attacked IS NOT NULL AND targets_attacked != '[]'
    GROUP BY jsonb_array_elements_text(targets_attacked::jsonb)
    HAVING COUNT(*) >= 2
  ),
  -- Summary metrics
  summary AS (
  ...
  recommendations AS (
    SELECT creative_id, ad_id, issue_primary, headline, thumbnail_url, creative_type,
           ...
           (SELECT jsonb_array_elements_text(targets_attacked::jsonb) LIMIT 1) as target_attacked
    FROM creative_performance
    ORDER BY roas DESC
    LIMIT 50
  ),
