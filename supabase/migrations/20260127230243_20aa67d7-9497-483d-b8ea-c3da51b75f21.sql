...
BEGIN
  ...
  creative_recommendations AS (
    SELECT cp.creative_id, cp.ad_id, cp.issue_primary, cp.headline, cp.thumbnail_url,
           ...
           (SELECT COALESCE(array_to_string(cp.targets_attacked, ', '), NULL)) as target_attacked
  ...
  donor_stats AS (
    SELECT
      COUNT(*) FILTER (WHERE at.amount < 100) as small_donor_count,
      COALESCE(SUM(at.amount) FILTER (WHERE at.amount < 100), 0) as small_donor_total,
      COUNT(*) FILTER (WHERE at.amount >= 100) as large_donor_count,
      COALESCE(SUM(at.amount) FILTER (WHERE at.amount >= 100), 0) as large_donor_total,
      ROUND(AVG(at.amount)::numeric, 2) as avg_donation,
      PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY at.amount) as median_donation,
      COUNT(DISTINCT at.donor_email) as total_donors,
  ...
  SELECT json_build_object(
    ...
    'donor_segmentation', (SELECT json_build_object(
      'small_donors', json_build_object('count', small_donor_count, 'total_amount', small_donor_total, 'percentage_of_total', ROUND((small_donor_total / NULLIF(small_donor_total + large_donor_total, 0) * 100)::numeric, 1)),
      'large_donors', json_build_object('count', large_donor_count, 'total_amount', large_donor_total, 'percentage_of_total', ROUND((large_donor_total / NULLIF(small_donor_total + large_donor_total, 0) * 100)::numeric, 1)),
    ...
    'leading_indicators', (SELECT json_build_object(
      'correlations', json_build_object(
        ...
        'ctr_correlation_pvalue', ctr_correlation_pvalue,
        'cpm_correlation_pvalue', cpm_correlation_pvalue,
        'ctr_correlation_significant', COALESCE(ctr_correlation_pvalue < p_significance_level, false),
        'cpm_correlation_significant', COALESCE(cpm_correlation_pvalue < p_significance_level, false)
      ),
      'sample_size', COALESCE(sample_size, 0),
      'avg_early_impressions', COALESCE(avg_early_impressions, 0),
      'insight', CASE
        WHEN sample_size >= 10 AND ABS(COALESCE(ctr_roas_correlation, 0)) >= 0.5 
          THEN 'Strong correlation detected - early CTR is a reliable predictor of final ROAS'
        WHEN sample_size >= 10 AND ABS(COALESCE(ctr_roas_correlation, 0)) >= 0.3
          THEN 'Moderate correlation - early CTR provides useful signal for ROAS prediction'
        WHEN sample_size < 10
          THEN 'Insufficient data for reliable correlation analysis'
        ELSE 'Weak correlation - early metrics are not strong predictors of final ROAS'
      END
    ) FROM leading_indicators),
    'fatigue_alerts', (SELECT COALESCE(json_agg(row_to_json(fa) ORDER BY fa.decline_from_peak DESC) FILTER (WHERE fa.fatigue_status IN ('FATIGUED', 'DECLINING')), '[]'::json) FROM fatigue_analysis fa),
    'recommendations', (SELECT COALESCE(json_agg(row_to_json(cr) ORDER BY cr.roas DESC), '[]'::json) FROM creative_recommendations cr),
    'recommendation_summary', (SELECT json_build_object(
    ...
    'data_quality', (SELECT json_build_object(
      ...
      'overall_confidence', CASE
        WHEN AVG(total_impressions) >= 10000 AND COUNT(*) >= 20 THEN 'HIGH'
        WHEN AVG(total_impressions) >= 5000 AND COUNT(*) >= 10 THEN 'MEDIUM'
        ELSE 'LOW'
      END
    ) FROM creative_performance),
