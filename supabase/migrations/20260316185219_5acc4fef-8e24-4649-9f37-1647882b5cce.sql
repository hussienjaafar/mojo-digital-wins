CREATE OR REPLACE FUNCTION public.get_donor_universe(_page integer DEFAULT 1, _page_size integer DEFAULT 50, _org_filter uuid[] DEFAULT NULL::uuid[], _state_filter text DEFAULT NULL::text, _min_amount numeric DEFAULT NULL::numeric, _max_amount numeric DEFAULT NULL::numeric, _recurring_filter boolean DEFAULT NULL::boolean, _crossover_only boolean DEFAULT false, _search text DEFAULT NULL::text, _channel_filter text DEFAULT NULL::text)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  WITH tx_stats AS (
    SELECT
      lower(trim(at.donor_email)) AS donor_email,
      at.organization_id,
