...
-- Fix materialized view to handle lowercase news topics and merge with proper case social topics
...
-- News trends - allow any case since extract-trending-topics uses lowercase
news_trends AS (
  SELECT 
    -- Convert to title case for display
    INITCAP(topic) as name,
...
-- Social trends (BlueSky) - these should be proper nouns already
social_trends AS (
  SELECT 
    topic as name,
    LOWER(TRIM(topic)) as normalized_name,
...
combined AS (
  SELECT 
    -- Prefer proper case name from social, fallback to news title case
    COALESCE(
      MAX(CASE WHEN source_type = 'social' THEN name END),
      MAX(CASE WHEN source_type = 'news' THEN name END)
    ) as name,
    normalized_name,
